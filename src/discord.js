...
exports.login = ({token, intents}) => {
    ...
    client.once(Events.ClientReady, (c) => {
        ...
        exports.sendMemberData = (connection, svId, memberId) => {
            const sv = client.guilds.cache.find(server => server.id === svId)
            sv.members.fetch(memberId).then(member => {
                member.user.fetch().then(user => {
                    ...
                    if(new_intents.includes(GatewayIntentBits.GuildPresences) && member.presence ) {
                    ...
                    for(const role of roles) {
                        ...
                        filtered_roles[role[1].name] = role[1].hexColor
                    }
        ...
        exports.sendRoleData = (connection, svId, roleId) => {
            const sv = client.guilds.cache.find(server => server.id === svId)
